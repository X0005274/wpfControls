...
| 클래스 | 역할 |
|---|---|
| `RendezvousSettings` | 접속 정보(Service/Network/Daemon) |
| `RendezvousConnection` | RV 환경 + Transport를 열고 닫음(IDisposable) |
| `RvMessage` | 이름=값 필드 메시지(문자열/정수/실수). `Method` 필드로 기능 구분 |
| `RendezvousRequestClient` | 요청을 보내고 응답을 기다림(동기) |
| `RendezvousReplyServer` | 요청을 듣고 처리해 응답을 보냄(백그라운드) |
| `RvMethodRouter` | Method 이름별 처리 함수 등록/분기 |
...
RendezvousSettings settings = new RendezvousSettings("7500", "", "tcp:7500");
using (RendezvousConnection connection = new RendezvousConnection(settings))
{
    RendezvousRequestClient client = new RendezvousRequestClient(connection);

    // 2) 요청 메시지를 만듭니다. Method 로 서버 기능을 지정합니다.
    RvMessage request = new RvMessage();
    request.Method = "GetEmployee";
    request.SetString("empNo", "E-10293");

    // 3) 주소(Subject)로 보내고 최대 5초간 응답을 기다립니다.
    RvMessage reply = client.Request("RPC.EMPLOYEE.GET", request, 5.0);
...
RendezvousSettings settings = new RendezvousSettings("7500", "", "tcp:7500");
RendezvousConnection connection = new RendezvousConnection(settings);

// 1) 기능(Method)별 처리 함수를 등록합니다.
RvMethodRouter router = new RvMethodRouter();
router.Register("GetEmployee", delegate (RvMessage request)
{
    string empNo = request.GetString("empNo");

    // (여기서 실제 업무 로직/DB 조회 수행)
    RvMessage reply = new RvMessage();
...
RendezvousReplyServer server = new RendezvousReplyServer(
...
- `RendezvousConnection` / `RendezvousReplyServer` 는 다 쓰면 반드시 `Dispose()` 하세요(`using` 권장).
