...
| 분류 | 컨트롤 |
|---|---|
| 입력 | `ModernTextBox`, `ModernPasswordBox`, `ModernRichTextBox`, `ModernSearchBox`, `ModernButton`, `ModernNumericUpDown`, `ModernMaskedTextBox`, `ModernCurrencyBox`, `ModernDateTimePicker`, `ModernTimePicker`, `ModernFilePicker` |
...
| 표시 | `ModernLabel`, `ModernProgressBar`, `ModernStatusBar`, `ModernBadge`, `ModernDatePicker`, `ModernDivider`, `ModernAvatar`, `ModernChip`, `ModernCard`, `ModernKpiCard` |
| 데이터 | `ModernDataGrid`, `MatteDataGrid`, `ModernListView`, `ModernTreeView`, `ModernTabControl` |
| 레이아웃 | `ModernExpander`, `ModernGroupBox`, `ModernFormSection`, `ModernScrollSection`, `ModernPagination`, `ModernBreadcrumb`, `ModernPageHeader`, `ModernSegmented`, `ModernStepIndicator` |

> **피드백(토스트·로딩·모달)** 은 드래그 컨트롤이 아니라 코드 호출용 헬퍼 API입니다.
> `com.example` 의 `Controls/Wpf/Feedback/README.md` (ModernToast, ModernDialog, LoadingOverlay)를 참고하세요.
